...
category_columns = ['zone', 'is_big_chance', 'is_final_third', 'is_key_pass',
                    'is_to_final_third', 'is_forward', 'is_cross',
                    'is_long', 'is_corner', 'is_freekick',
                    'is_through', 'is_short']
...
def parse_data(data: list):
    ...
    for item in data:
        ...
        new_item['result'] = str(item['is_success'])
        result.append(new_item)
    return result


xsp_model_dir = 'xsp_model'
if xsp_model_dir not in os.listdir():
    xsp_model_dir = os.path.join('nn_xg', xsp_model_dir)
xsp_model = tf.estimator.DNNClassifier(
    model_dir=xsp_model_dir,
...
def predict_success_passes(origin_test_data):
    ...
    # evaluation_result = xsp_model.evaluate(input_fn=test_input_fn)
    predictions = list(xsp_model.predict(input_fn=test_input_fn))
    for i, prediction in enumerate(predictions):
        origin_test_data[i]['xsp'] = prediction['probabilities'][1]
    df = pd.DataFrame(origin_test_data)
    print(df.groupby(['team_id']).sum()[['is_success', 'xsp']].sort_values(['is_success']))
    return df


def predict():
    origin_test_data = load_json('test_passes.json')
    predict_success_passes(origin_test_data)
